...
int printDist(int a,int b){
    ...
    if(arr[k]!=arr_2[k]){
      final_arr[k]=arr[k];
      final_arr[k+1]=arr_2[k];


    }
    for(int m=0;m<=20;m++){

    int count=final_arr[m];
...
int main(void)
{
    ...
    while(1)
    {
        ...
        printf("11. calc node depth 12.calcDepth with item  13.rangesearch 14. delete item  15.printDist\n\n");
        int ch;
        scanf("%d",&ch);
        if(ch==1)
        {
